...
def train_or_eval_graph_model(model,audiomodel, visualmodel, textmodel, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
    ...
    for data in dataloader:
        ...
        if train:
            ...
            CE_audio = F.nll_loss(pseudo_audio, label)
            CE_visual = F.nll_loss(pseudo_visual, label)
            CE_text = F.nll_loss(pseudo_text, label)
            loss = CE_audio+CE_visual+CE_text
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default=400, metavar='E', help='number of epochs')
    ...
    optimizer = optim.Adam(list(model.parameters())+list(audiomodel.parameters())+list(visualmodel.parameters())+list(textmodel.parameters()), lr=args.lr, weight_decay=args.l2)
    ...
    if args.av_using_lstm:
        model_save_dir = os.path.join("/workspace/MGLRA/save_folder", args.Dataset, "MKD_online_av_using_lstm")
        os.makedirs(model_save_dir, exist_ok=True)
    else:
        model_save_dir = os.path.join("/workspace/MGLRA/save_folder", args.Dataset, "MKD_online")
        os.makedirs(model_save_dir, exist_ok=True)
    ...
    for e in range(n_epochs):
        ...
        if (best_fscore < test_fscore):
            ...
            if args.av_using_lstm:
                result_dictionary["mode"]="MKD_online_AV_LSTM"
            else:
                result_dictionary["mode"]="MKD_online"
